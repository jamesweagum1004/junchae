...
import {
  FolderOpen, List, Image, Cpu, Megaphone, BarChart2, FileText, Radio,
  LogOut, Zap, ChevronRight, Menu, X, Shield, KeyRound,
} from 'lucide-react';
...
const TABS = [
  ...
  { id: 'sites',      label: '사이트 리스트', icon: List,       desc: '상태 관리' },
  { id: 'logos',      label: '로고/파비콘',   icon: Image,      desc: '이미지 업로드' },
  { id: 'deepseek',   label: 'DeepSeek AI',  icon: Cpu,        desc: 'API 연동' },
  { id: 'ads',        label: '광고 컨트롤러', icon: Megaphone,  desc: '배너 관리' },
  { id: 'analytics',  label: '구글 SiteKit', icon: BarChart2,   desc: '메트릭스' },
  { id: 'pseo',       label: 'pSEO 관리',    icon: FileText,   desc: '상세페이지' },
  { id: 'bridge',     label: '브릣지 광고',   icon: Radio,      desc: 'ExoClick 등' },
  { id: 'account',    label: '보안 / 계정 설정', icon: KeyRound, desc: 'ID·PW·주로 변경' },
];
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        >
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-left transition-all duration-150 group ${
                    isActive
                      ...
                      : 'hover:bg-obsidian-700 border border-transparent'
                  }`}
                >
                  <Icon
                    size={15}
                    className={`flex-shrink-0 ${isActive ? 'text-neon-orange' : 'text-slate-500 group-hover:text-slate-300'}`}
                  />
          ...
          <div className="p-3 border-t border-obsidian-600">
            <div className="px-3 py-2.5 bg-obsidian-700 rounded-lg">
              <p className="text-[10px] font-mono text-slate-600">BUILD</p>
              <p className="text-xs font-bold text-slate-400 font-mono">전체닷컴 v2.1.0</p>
              <p className="text-[10px] text-slate-700 font-mono mt-0.5">Secure Admin Panel</p>
            </div>
          </div>
        </aside>
